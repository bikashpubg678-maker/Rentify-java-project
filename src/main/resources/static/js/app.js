...
function sendMessage() {
    ...
    fetch('/api/chat', {
        ...
        body: JSON.stringify({ messages: chatHistory, language: language })
    })
...
// The actual system prompt is built on the server (it needs live DB data and
// the user's language choice). This client-side placeholder is kept only so
// the very first render has *something* in the history; the server will
// overwrite it before calling OpenRouter.
const SYSTEM_PROMPT = "You are the official AI assistant for Rentify, a car rental web application built by Bikash Talukder. You are bilingual (English + Bengali / বাংলা) and reply in the user's selected language.";

// ── Language state (persisted in localStorage) ─────────────────────────────
const LANG_STORAGE_KEY = 'rentify-chat-lang';
...
function setChatLanguage(lang) {
    ...
    // Toggle the visible button states
    const enBtn = document.getElementById('chatbot-lang-en');
    const bnBtn = document.getElementById('chatbot-lang-bn');
    if (enBtn) enBtn.classList.toggle('active', lang === 'en');
    if (bnBtn) bnBtn.classList.toggle('active', lang === 'bn');

    // Show/hide localized welcome text + hint chips
    document.querySelectorAll('[data-lang]').forEach(el => {
        el.style.display = (el.getAttribute('data-lang') === lang) ? '' : 'none';
    });

    // Swap the input placeholder
    const input = document.getElementById('chatbot-input');
    if (input) {
        const ph = input.getAttribute(lang === 'bn' ? 'data-placeholder-bn' : 'data-placeholder-en');
...
// ── DOMContentLoaded ─────────────────────────────────────────────────────────
document.addEventListener('DOMContentLoaded', () => {
    // Apply persisted chatbot language (default: en)
    setChatLanguage(getChatLanguage());
